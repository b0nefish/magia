...
errorstyle_t error_style = ERRORSTYLE_VASM;
...
static void format_error_vasm(FILE* f, int n,va_list vl,struct err_out *errlist,int offset)
{
  ...
  if (!(flags & NOLINE) && cur_src!=NULL)
    fprintf(f," in line %d of \"%s\"",TBL_SRC_LINE(cur_src),TBL_SRC_NAME(cur_src));
  fprintf(f,": ");
  ...
  if (!(flags & NOLINE) && cur_src!=NULL) {
    if (cur_src->parent != NULL) {
      ...
      while (parent = child->parent) {
        ...
        fprintf(f," from line %d of \"%s\"",child->parent_line,TBL_SRC_NAME(parent));

        recurs = 1;
        while (parent->parent!=NULL &&
               child->parent_line==parent->parent_line &&
               !strcmp(parent->real_name,parent->parent->real_name)) {
          recurs++;
...
static void format_error_modern(FILE* f, int n,va_list vl,struct err_out *errlist,int offset,errorstyle_t style)
{
  const int   flags    = errlist[n].flags;
  const char *src      = TBL_SRC_NAME(cur_src);
  const int   lineno   = TBL_SRC_LINE(cur_src);
  const char *kind     = (flags &ERROR) ? "error" : (flags &WARNING) ? "warning" : "message";
  const int   error_no = n + offset;

  if (flags & ERROR)
    ++errors;

  if (flags & NOLINE) {
    fprintf(f, "%s: %s %d: ", src, kind, error_no);
  }
  else {
    if (style == ERRORSTYLE_GCC)
      fprintf(f, "%s:%d: %s %d: ", src, lineno, kind, error_no);
    else
      fprintf(f, "%s(%d): %s C%04x: ", src, lineno, kind, error_no);
  }
...
static void error(int n,va_list vl,struct err_out *errlist,int offset)
{
  ...
  if ((flags&MESSAGE) && !(flags&(WARNING|ERROR|FATAL))) {
    ...
    if (last_err_source) {
      ...
      if (cur_src!=NULL && cur_src==last_err_source &&
         cur_src->real_line==last_err_line &&
  ...
  if (cur_src) {
    ...
    last_err_line = cur_src->real_line;
    last_err_no = n + offset;
  }

  switch (error_style) {
    case ERRORSTYLE_VASM:
      format_error_vasm(f, n, vl, errlist, offset);
      break;
    default:
      format_error_modern(f, n, vl, errlist, offset, error_style);
      break;
  }
...
void output_atom_error(int n,atom *a,...)
{
  ...
  cur_src = a->src;
  cur_src->real_line = a->real_line;
  cur_src->override_line = a->override_line;
  error(n,vl,output_err_out,FIRST_OUTPUT_ERROR);
  cur_src = old;
}
