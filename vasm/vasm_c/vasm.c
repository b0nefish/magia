...
extern int hunk_onlyglobal; /* TBL addition */
extern int hunk_includelocal; /* TBL addition */
...
static void resolve_section(section *sec)
{
  ...
  do{
    ...
    for(p=sec->first;p;p=p->next){
      ...
      cur_src=p->src;
      cur_src->real_line=p->real_line;
      cur_src->override_line=p->override_line;
#if HAVE_CPU_OPTS
      if(p->type==OPTS){
...
static void assemble(void)
{
  ...
  for(sec=first_section;sec;sec=sec->next){
    ...
    for(p=sec->first;p;p=p->next){
      ...
      if(sec->pc!=basepc){
        atom *aa;
        if (p->type==LABEL&&p->next!=NULL&&p->next->real_line==p->real_line)
      ...
      if(p->type==RORG){
        ...
        if(lasterrsrc!=p->src||lasterrline!=p->real_line){
          general_error(31);  /* initialized data in bss */
          lasterrsrc=p->src;
          lasterrline=p->real_line;
        }
      }
#if HAVE_CPU_OPTS
      else if(p->type==OPTS)
...
int vasm_main(int argc,char **argv)
{
  int i;
  for(i=1;i<argc;i++){
  ...
  for(i=1;i<argc;i++){
    ...
    if (!strncmp("-efm=", argv[i], 5)) {
      const char* option = argv[i] + 5;
      if (0 == strcmp("vasm", option)) {
        /* already the default */
      }
      else if (0 == strcmp("gcc", option)) {
        error_style = ERRORSTYLE_GCC;
      }
      else if (0 == strcmp("msvc", option)) {
        error_style = ERRORSTYLE_MSVC;
      }
      else {
...
source *new_source(char *filename,char *text,size_t size)
{
  ...
  s->parent = cur_src;
  s->parent_line = TBL_SRC_LINE(cur_src);
  s->real_name = mystrdup(filename);
  s->override_name = NULL;
  s->text = text;
  s->size = size;
  s->macro = NULL;
...
void set_list_title(char *p,int len)
{
  ...
  listtitlelines[listtitlecnt-1]=cur_src->real_line;
}
...
void write_listing(char *listname)
{
  ...
  for(p=first_listing;p;p=p->next){
    ...
    while(a){
      ...
      if(a->next&&a->next->real_line==a->real_line&&a->next->src==a->src){
        a=a->next;
        pc=pcalign(a,pc);
      }else
        a=0;
