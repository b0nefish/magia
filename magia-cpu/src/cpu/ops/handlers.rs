...
macro_rules! op_entry {
    ($mask:expr, $matching:expr, $handler:ident) => {
        OpcodeHandler {
            mask: $mask,
            matching: $matching,
            handler: $handler,
        ...
        G: for<'a> Fn(&OpcodeHandler<T>) -> F,
    {
        ...
        for op in &self.optable {
            match offset_cache.get(&op.mask) {
                Some(offsets) => {
                    for opcode in offsets
                        .iter()
                        .flat_map(|&(start, len)| (start..(start + len)).map(|o| o + op.matching))
                    {
